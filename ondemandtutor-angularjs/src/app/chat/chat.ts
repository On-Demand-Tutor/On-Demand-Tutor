...
avatar: undefined
  ...
  private async trySetupWebSocket(tutorId: number): Promise<void> {
    ...
    try {
      const r1 = await this.http.get<any>(`${this.chatsMsgBase}/room-id/${tutorId}`, { headers: this.buildHeaders() }).toPromise();
      roomId = r1?.roomId ?? null;
    } catch {}
    if (!roomId) {
      try {
        const r2 = await this.http.get<any>(`${this.chatFallbackBase}/room-id/${tutorId}`, { headers: this.buildHeaders() }).toPromise();
roomId = r2?.roomId ?? null;
    ...
    const [Stomp, SockJSMod] = await Promise.all([import('stompjs'), import('sockjs-client')]);
    ...
    const socket = new (SockJS as any)(endpoint);
    const client = (Stomp as any).over(socket);
